...
/**
 * Represents an error entry which contains a Siddhi Event Array.
 */
public class EventArrayErrorEntry extends AbstractErrorEntry {
    private Event[] event;

    public EventArrayErrorEntry(int id, long timestamp, String siddhiAppName, String streamName, String cause,
                                String stackTrace, String originalPayload, ErrorOccurrence errorOccurrence,
                                ErrorType errorType, Event[] event) {
        super(id, timestamp, siddhiAppName, streamName, cause, stackTrace, originalPayload, errorOccurrence,
                ErroneousEventType.EVENT_ARRAY, errorType);
        this.event = event;
    }
