...
public class SinhFunctionExtensionTestCase {
    private static Log logger = LogFactory.getLog(SinhFunctionExtensionTestCase.class);
    protected static SiddhiManager siddhiManager;
    private boolean eventArrived;

    @Test
    public void testProcess() throws Exception {
        logger.info("SinhFunctionExtension TestCase");

        siddhiManager = new SiddhiManager();
        String inValueStream = "define stream InValueStream (inValue double);";

        String eventFuseExecutionPlan = ("@info(name = 'query1') from InValueStream "
                + "select math:sinh(inValue) as sinValue "
        ...
        executionPlanRuntime.addCallback("query1", new QueryCallback() {
            ...
            public void receive(long timeStamp, Event[] inEvents,
                ...
                for (Event event : inEvents) {
                    ...
                    Assert.assertEquals((Double) (201.71315737027922), result);
                }
            }
        });
    ...
    public void exceptionTestCase1() throws Exception {
        logger.info("SinhFunctionExtension exceptionTestCase1");

        siddhiManager = new SiddhiManager();
        String inValueStream = "define stream InValueStream (inValue double);";

        String eventFuseExecutionPlan = ("@info(name = 'query1') from InValueStream "
                                         + "select math:sinh(inValue,inValue) as sinValue "
                                         + "insert into OutMediationStream;");
        ExecutionPlanRuntime executionPlanRuntime = siddhiManager.createExecutionPlanRuntime(inValueStream +
                                                                                             eventFuseExecutionPlan);
    }
    ...
    public void exceptionTestCase2() throws Exception {
        logger.info("SinhFunctionExtension exceptionTestCase2");
    ...
    public void testProcessInteger() throws Exception {
        logger.info("SinhFunctionExtension testProcessInteger");
    ...
    public void testProcessLong() throws Exception {
        logger.info("SinhFunctionExtension testProcessLong");
        ...
        executionPlanRuntime.addCallback("query1", new QueryCallback() {
            ...
            public void receive(long timeStamp, Event[] inEvents,
                ...
                for (Event event : inEvents) {
                    result = (Double) event.getData(0);
                    Assert.assertEquals((Double) (1.8383670615634575E270), result);
                }
            }
        });
        InputHandler inputHandler = executionPlanRuntime
                .getInputHandler("InValueStream");
        executionPlanRuntime.start();
        inputHandler.send(new Long[]{623l});
        Thread.sleep(100);
        executionPlanRuntime.shutdown();
    }
    ...
    public void testProcessFloat() throws Exception {
        logger.info("SinhFunctionExtension testProcessFloat");
        ...
        executionPlanRuntime.addCallback("query1", new QueryCallback() {
            ...
            public void receive(long timeStamp, Event[] inEvents,
                ...
                for (Event event : inEvents) {
                    result = (Double) event.getData(0);
                    Assert.assertEquals((Double) (2.5879283518955854E270), result);
                }
            }
        });
        InputHandler inputHandler = executionPlanRuntime
                .getInputHandler("InValueStream");
        executionPlanRuntime.start();
        inputHandler.send(new Float[]{623.342f});
        Thread.sleep(100);
        executionPlanRuntime.shutdown();
    }
}
