...
public class FloorFunctionExtensionTestCase {
    private static Log logger = LogFactory.getLog(FloorFunctionExtensionTestCase.class);
    ...
    public void testProcessFloat() throws Exception {
        ...
        executionPlanRuntime.addCallback("query1", new QueryCallback() {
            ...
            public void receive(long timeStamp, Event[] inEvents,
                ...
                for (Event event : inEvents) {
                    result = (Double) event.getData(0);
                    Assert.assertEquals((Double) 1023.0, result);
                }
            }
        });
        InputHandler inputHandler = executionPlanRuntime
    ...
    public void testProcessLong() throws Exception {
        ...
        executionPlanRuntime.addCallback("query1", new QueryCallback() {
            ...
            public void receive(long timeStamp, Event[] inEvents,
                ...
                for (Event event : inEvents) {
                    result = (Double) event.getData(0);
                    Assert.assertEquals((Double) 102342.0, result);
                }
            }
        });
        InputHandler inputHandler = executionPlanRuntime
