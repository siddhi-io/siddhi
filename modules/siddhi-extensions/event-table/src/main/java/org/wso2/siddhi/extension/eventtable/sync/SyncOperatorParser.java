 ...
 * Created on 1/19/15.
 */
public class SyncOperatorParser {
    ...
    public static Operator parse(Expression expression, MetaComplexEvent matchingMetaComplexEvent, ExecutionPlanContext executionPlanContext, List<VariableExpressionExecutor> variableExpressionExecutors,
                                 Map<String, EventTable> eventTableMap, int matchingStreamIndex, AbstractDefinition candidateDefinition, long withinTime, String indexedAttribute, int indexedPosition, SyncTableHandler syncTableHandler) {

        if (indexedAttribute == null) {
            return SyncOperatorParser.parse(expression, matchingMetaComplexEvent, executionPlanContext, variableExpressionExecutors, eventTableMap, matchingStreamIndex, candidateDefinition, withinTime, indexedPosition);
        }
        ...
        if (expression instanceof Compare && ((Compare) expression).getOperator() == Compare.Operator.EQUAL) {
            ...
            if ((compare.getLeftExpression() instanceof Variable || compare.getLeftExpression() instanceof Constant)
                ...
                if (leftSideIndexed && !rightSideIndexed) {
                    ...
                    return new SimpleSyncIndexedOperator(expressionExecutor, matchingStreamIndex, withinTime, matchingStreamDefinition.getAttributeList().size(), indexedPosition, syncTableHandler);
    ...
    public static Operator parse(Expression expression, MetaComplexEvent matchingMetaComplexEvent, ExecutionPlanContext executionPlanContext, List<VariableExpressionExecutor> variableExpressionExecutors,
        ...
        return SyncOperatorParser.parse(expression, matchingMetaComplexEvent, executionPlanContext, variableExpressionExecutors, eventTableMap, matchingStreamIndex, candidateDefinition, withinTime, 0);
    }
