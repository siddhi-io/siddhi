...
public class IndexedTableTestCase {
    ...
    @Test
    public void indexedTableTest0() throws InterruptedException {
        log.info("indexedTableTest0");

        SiddhiManager siddhiManager = new SiddhiManager();
        String executionPlan = "define stream EligibilityStream (rule string, messageID string, isEligible bool, isLocallyThrottled bool, throttle_key string);\n" +
                "\n" +
                ...
                "\n" +
                "FROM EligibilityStream[isEligible==true AND isLocallyThrottled==false]#window.length(1) LEFT OUTER JOIN ThrottleTable\n" +
                "ON ThrottleTable.THROTTLE_KEY == EligibilityStream.throttle_key\n" +
                "SELECT rule, messageID, ifThenElse((ThrottleTable.THROTTLE_KEY is null),false, true) AS isThrottled\n" +
        ...
        try {
            ...
            InputHandler eligibilityStream = executionPlanRuntime.getInputHandler("EligibilityStream");
            executionPlanRuntime.start();
            eligibilityStream.send(new Object[]{"RULE", "message1", true, false, "IBM"});
            eligibilityStream.send(new Object[]{"RULE", "message2", true, false, "WSO2"});
            eligibilityStream.send(new Object[]{"RULE", "message3", true, false, "WSO2"});
