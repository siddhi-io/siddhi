...
!!! Note "Including database drivers."
    The database driver corresponding to the database should be an OSGi bundle and it need to be added to <code>&lt;SIDDHI_RUNNER_HOME&gt;/lib/</code> directory. If the driver is a jar then this should be [converted to an OSGi bundle before adding](http://siddhi.io/documentation/siddhi-5.x/config-guide-5.x/#converting-jars-to-osgi-bundles).
...
The ref configured in Siddhi Configuration yaml can be referred from a Siddhi Application [Source](http://siddhi-io.github.io/siddhi/documentation/siddhi-5.x/query-guide-5.x/#source) as follows.
```
@Source(ref='<name>',
        @map(type='json', @attributes( name='$.name', amount='$.quantity')))
define stream SweetProductionStream (name string, amount double);
```
Similarly [Sinks](http://siddhi-io.github.io/siddhi/documentation/siddhi-5.x/query-guide-5.x/#sink) and [Store Tables](http://siddhi-io.github.io/siddhi/documentation/siddhi-5.x/query-guide-5.x/#store) can also be configured and referred from Siddhi Apps.
...
Refer [guide](https://siddhi-io.github.io/siddhi/documentation/siddhi-5.x/user-guide-5.x/#using-siddhi-as-a-java-library) for more details on using Siddhi as a Java Library.
...
To enable statistics, the relevant configuration should be added to the Siddhi Configuration yaml as follows, and at the same time the statistics collection should be enabled in the Siddhi Application which is being monitored. Refer [Siddhi Application Statistics](https://siddhi-io.github.io/siddhi/documentation/siddhi-5.x/query-guide-5.x/#statistics) documentation for enabling Siddhi Application level statistics.
...
The bellow sample is referring to the datasource with JNDI name `jdbc/SiddhiMetricsDB`, hence the datasource configuration in yaml should have `jndiConfig.name` as `jdbc/SiddhiMetricsDB`. For detailed instructions on configuring a datasource, refer [Configuring Databases](http://siddhi.io/documentation/siddhi-5.x/config-guide-5.x/#configuring-databases).
